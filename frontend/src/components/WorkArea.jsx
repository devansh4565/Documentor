...
    const checkMindMapStatus = async () => {
        // ✅ FIX: Check for selectedFile and its _id before making the API call.
        if (!selectedFile?._id) {
...
    if (acceptedFiles.length === 0 || !firebaseUser || !selectedChat) {
        if (!selectedChat) alert("Please select a chat session before uploading a file.");
        return;
    }
...
        });
        
        // res.data is now the full file object from the database, thanks to our backend fix.
...
        alert(`Upload Error: ${err.response?.data?.message || err.message || 'An unknown error occurred'}`);
    } finally {
...
                const finalBotMessage = { _id: `temp-bot-${Date.now()}`, sender: 'assistant', text: botResponseText };
...
                    authHeader
                ).catch(err => { console.error("Background save of bot message failed:", err); });
            }
        }, 30);
...
        await axios.post( `${API}/api/mindmap/file/${selectedFile._id}`, { data: mindMapData });
...
    } catch (err) {
        console.error("Failed to generate/save mind map:", err);
        alert("Error generating mind map. Please check the console.");
    } finally {
        setLoading(false);
    }
}, [selectedFile, API, navigate, setLoading]);
...
            if (file.content) {
                const summary = await sendToGPT(file.content);
...
            } else {
                clearInterval(typingInterval);
                const finalBotMessage = { _id: `bot-multi-sum-${Date.now()}`, sender: 'assistant', text: combinedSummary };
                setMessages(prev => [...prev, finalBotMessage]);
                setBotTyping("");
...
                        { headers: { Authorization: `Bearer ${token}` } }
                    ).catch(err => { console.error("Background save of multi-summary message failed:", err); });
                });
            }
        }, 30);
...
}, [messages, initialSessions, selectedChat]);
...
  const rightPanel = (
    <>
      {/* ✅ FIX: Moved Theme Toggle button here */}
      <div className="flex-shrink-0 p-4 border-b dark:border-gray-700 flex justify-between items-center">
        <h2 className="text-xl font-bold">Chat</h2>
        <div className="flex items-center gap-4">
            <button
                onClick={toggleTheme}
                className={`p-2 rounded-full shadow hover:scale-105 transition-all duration-300 ring-2 ring-offset-2 ${
...
                        <button onClick={handleMultiFileSummarize} className="flex items-center gap-2 px-4 py-2 bg-green-500 text-white font-semibold rounded-full shadow-lg hover:bg-green-600">
                            <Lightbulb size={18} />
                            <span> Summarize {selectedFilesForSummary.length} File{selectedFilesForSummary.length > 1 ? 's' : ''} </span>
                        </button>
                    </motion.div>
                )}
            </AnimatePresence>
            {selectedChat && (
                mindMapExists ? (
                    <button onClick={() => navigate("/mindmap", { state: { fileId: selectedFile._id } })} className="flex items-center gap-3 px-5 py-3 bg-purple-600 text-white font-semibold rounded-full shadow-lg hover:bg-purple-700">
                        <span>View Mind Map</span>
                    </button>
                ) : (
                    <button onClick={handleGenerateMindMap} disabled={loading} className="flex items-center gap-3 px-5 py-3 bg-blue-600 text-white font-semibold rounded-full shadow-lg hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed">
